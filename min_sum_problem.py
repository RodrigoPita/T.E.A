...
def show(piramide):
    for i in range(0, len(piramide)):
        print(' '*(len(piramide) - i), piramide[i])
    return

def min_sum_bottom_up(piramide):
    start = time()
    # tabela para preencher com os subproblemas
    memo = [None] * len(piramide)
    n = len(piramide) - 1
     
    # colocando a base da piramide no memo
    for i in range(len(piramide[n])):
        memo[i] = piramide[n][i]
 
    # calculando o resto da tabela
    for i in range(len(piramide) - 2, -1,-1):
        for j in range( len(piramide[i])):
            memo[j] = piramide[i][j] + min(memo[j],
                                    memo[j + 1]);
...
while True:
    ...
    #show(piramide)
    min_sum_top_down(piramide)
    min_sum_bottom_up(piramide)
